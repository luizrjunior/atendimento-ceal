@php
    $data_inicio = date('d/m/Y', strtotime($bloqueio->data_inicio));
    $data_fim = date('d/m/Y', strtotime($bloqueio->data_fim));
@endphp
...
@section('javascript')
    <script>
        ...
        top.valorSelectAtividade = '{{ $bloqueio->horario->atividade_id }}';
        top.valorSelectHorario = '{{ $bloqueio->horario_id }}';

        $('#atividade_id').val(top.valorSelectAtividade);
        $('#horario_id').val(top.valorSelectHorario);
    </script>
            ...
            <div class="col-md-8">
                <div class="card uper">
                    ...
                    <div class="card-body">
                        ...
                        <form method="post" action="{{ route('bloqueios.update', $bloqueio->id) }}">
                            @method('PATCH')
                            @csrf

                            <div class="form-group">
                                <label for="nome">{{ __('Name') }}</label>
                                <input id="nome" type="text" class="form-control @error('nome') is-invalid @enderror maiuscula" name="nome" value="{{ $bloqueio->nome }}" required autocomplete="nome">
                                @error('nome')
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control rounded-0" name="descricao" id="descricao" rows="3">{{ $bloqueio->descricao }}</textarea>
                            </div>
